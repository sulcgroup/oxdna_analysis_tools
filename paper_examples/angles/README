This folder contains example files to perform angle comparison between two DNA wireframes from Han et. al. (2017).  This example will use oxDNA to run a simulation and then use duplex_angle_finder.py to extract duplex information from the trajectory and duplex_angle_plotter.py to make a plot like Figure 7 in the paper.
...
2. Fit vectors to the duplexes in the simulation
   ...
   A couple notes on the duplex_angle_finder script:
     This script uses the hydrogen bonding potential to determine if two nucleotides are bonded, which is unaffected by fix diffusion.  You do not need to set the reference particle for this script.
     If you are running on a computer with multiple CPUs, you can additionally specify the -p <number> option to fit duplexes in parallel using tht many CPUs.  This results in significant performance gains up to around 30 cpus.
