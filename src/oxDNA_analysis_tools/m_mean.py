...
def compute(top, traj,centered_ref_coords, cms_ref_cords, ntopart, ptr):
    ...
    confs = [parse_conf(c,reader.top_info.bases) for c in confs]
    # convert to numpy repr
    aligned_coords = np.asarray([[align_conf.positions, align_conf.a1s, align_conf.a3s] for align_conf in confs])
